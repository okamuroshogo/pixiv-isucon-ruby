...
module Isuconp
  class App < Sinatra::Base
    ...
    post '/' do
      ...
      if params['file']
        ...
        open(file_path, "wb") do |output|
          output.write(params["file"][:tempfile].read)
        end
    ...
    get '/image/:id.:ext' do
      ...
      dir = "../public/image"
      ext = ""
      post = db.prepare('SELECT id, mime FROM `posts` WHERE `id` = ?').execute(params[:id].to_i).first
      ...
      if (params[:ext] == "jpg" && post[:mime] == "image/jpeg") ||
        ...
        file = File.open("#{dir}/image/#{post[:id]}.#{post[:ext]}", 'r+b')
        data = file.read
        file.close
        return data
      end
